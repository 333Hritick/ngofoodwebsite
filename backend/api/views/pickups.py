    ...
    def get_queryset(self):
        ...
        if user.role == 'ngo':
            return Pickup.objects.filter(ngo=user).select_related(
                'donation__donor__profile', 'ngo__profile'
            )

        elif user.role == 'donor':
            return Pickup.objects.filter(donation__donor=user).select_related(
                'donation__donor__profile', 'ngo__profile'
            )
...
class ClaimDonationView(APIView):
    ...
    def post(self, request):
        ...
        if not donation_id:
            return Response({'error': 'donation_id required'}, status=400)
        ...
        if donation.status != 'available':
            return Response({'error': 'Donation not available'}, status=400)
